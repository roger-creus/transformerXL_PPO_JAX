...
class ActorCriticTransformer(nn.Module):
    ...
    def __call__(self, memories,obs,mask):
        ...
        x= self.transformer(memories,obs,mask)
        #x=obs
    ...
    def model_forward_eval(self, memories,obs,mask):
        ...
        #x=obs
        #memory_out=jnp.zeros((memories.shape[0],)+memories.shape[2:])
        actor_mean = self.actor_ln1(x)
        actor_mean = self.activation_fn(actor_mean)
        actor_mean = self.actor_ln2(actor_mean)
        actor_mean = self.activation_fn(actor_mean)
...
def make_train(config,rng):
    ...
    def train(rng,train_state):
        ...
        def _update_step(runner_state, unused):
            ...
            #(train_state, env_state,memories_previous,memories_mask,memories_mask_idx, last_obs,done,_, _rng)=runner_state 
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch,memories_batch, gae, targets):
                        ...
                        # USE THE CACHED MEMORIES ONLY FROM THE FIRST STEP OF A WINDOW GRAD Because all other of the window will be computed again here.
                        ...
                        #CREATE THE MASK FOR WINDOW GRAD (have to take the one from the batch and roll them to match the real steps
                        ...
                        #roll of different value for each step to match the right step
                    ...
                    total_loss, grads = jax.lax.pmean(grad_fn(
